...
Afterwards, we introduce confounding by generating an artificial dataset with a predefined (truncated) Poisson distribution of baseline EDSS. The remaining covariate data are then generated conditional on the previously observed data distribution by means of multiple imputation.  The new dataset has a total of `r nrow(sim_NRS)` patients, with `r nrow(subset(sim_NRS, Trt == 0))` patients receiving control and `r nrow(subset(sim_NRS, Trt == 1))` patients receiving 'active' placebo. 
