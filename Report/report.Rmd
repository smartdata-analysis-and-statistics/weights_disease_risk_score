...
dat_baseline <- dat %>% group_by(USUBJID.x, TRIAL) %>% 
  filter(AVISITN == 0) %>%
  summarize(
    AGE = first(AGE),
...
ggdat <- data.frame(x = c(dat_baseline$AGE, 
                    ...
                    var = c(rep("Age", npat), 
                            rep("Weight", npat), 
                            rep("Time since first symptoms", npat),
                            rep("Disease duration (years)", npat),
                            rep("Baseline EDSS", npat),
                            rep("No. of Relapses (12mo)", npat),
                            rep("No. of Relapses (36mo)", npat),
                            rep("Number of Gd+ lesions", npat),
                            rep("Number of T1 lesions", npat),
                            rep("Number of T2 lesions",npat)),
