...
##############################################################################  
start.tdw <- proc.time()
dat_ASP$tdw <- tdw("TRT","pgs",dat_ASP, estimand = "att")
TDW <- glm(y~TRT, weights = tdw, data = dat_ASP)
t.tdw <- (proc.time() - start.tdw)[1]
  
results <- results %>% add_row(data.frame("Method"  = "DSR TDW",
                                          "Estimate" = coef(TDW)["TRT"],
                                          "SE" = sqrt(diag(vcov(TDW)))["TRT"],
                                          "CIL" = confint(TDW)["TRT",]["2.5 %"],
                                          "CIU" = confint(TDW)["TRT",]["97.5 %"],
                                          "Time" = t.tdw))
